...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  while(*pt){
      ...
      ST7735_DrawCharS(x*6, y*10, *pt, textColor, 0x39E6, 1);
    pt++;
    x = x+1;
...
//------------ST7735_DrawString2------------
uint32_t ST7735_DrawString2(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
      //ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
      ST7735_DrawCharS(x*12, y*10, *pt, textColor, 0x39E6, 2);
    pt++;
    x = x+1;
    if(x>20) return count;  // number of characters printed
    count++;
